...
```{r fit, cache.extra = file.info("../models/scorediff-oneseason_2011-fit.rds")}
fit <- readRDS("../models/scorediff-oneseason_2011-fit.rds")
```
...
color_scheme_set("mix-darkgray-purple")
...
```{r glog, cache.extra = file.info("lag/log-with-lags-cleaned.csv")}
...
ggplot(avals, aes(x = fct_rev(team))) +
    ...
    geom_linerange(aes(ymin = p10, ymax = p90), color = tc$background_dark) +
...
```{r scorediff_sim, dependson = c("matchups", "samps")}
df <- 7
...
set.seed(16125)
for (i in 1:n_iter){
    scorediff_sim[i,] <- samps$a[i, matchups$home] - samps$a[i, matchups$away] +
         rt(n_games, df) * samps$sigma_y[i]
}

result <- cbind(matchups, trace_intervals(scorediff_sim, "game_idx"))  %>%
    as_tibble() %>%
