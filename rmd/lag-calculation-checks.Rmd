...
lag %>%
    ...
    scale_color_manual(values = c("#000000", "#a9a9a9"), guide = FALSE) +
...
lag2011 %>%
    ggplot(aes(date, lag)) +
    geom_hline(yintercept = 0, color = "red", alpha = 0.2, size = 1.25) +
    geom_line(alpha = 0.6) +
    geom_point(size = 0.7) +
...
calculations or the code they used to generated these.  Using their
two supplemental tables, however, I can gain some idea of whether my
...
```{r, dependson = "lag_tally"}
lag_tally %>%
    group_by(field, direction, abs(lag)) %>%
...
Seattle due to a U2 concert."  OK, I'll have to add a processing step
to adjust for that.
...
By and large, however, it seems like my generated data set has lag
values that are consistent with SSA's data set.
