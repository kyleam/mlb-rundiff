...
```{r glog, dependson = "clean_team_name", cache.extra = tools::md5sum("../lag/log-with-lags-cleaned.csv")}
...
```{r info, cache.extra = tools::md5sum("../models/rundiff-lagwe_1992-2011.info.R")}
info <- read_rdump("../models/rundiff-lagwe_1992-2011.info.R")
```

```{r dat, cache.extra = tools::md5sum("../models/rundiff-lagwe_1992-2011.data.R")}
dat <- read_rdump("../models/rundiff-lagwe_1992-2011.data.R")
```
...
```{r fit, cache.extra = tools::md5sum("../models/rundiff-lagwe_1992-2011-fit.rds")}
fit <- readRDS("../models/rundiff-lagwe_1992-2011-fit.rds")
```
...
```{r sim_rgames, cache.extra = tools::md5sum("../models/rundiff-lagwe_1992-2011-sim-rgames.rds")}
sim_rgames <- readRDS("../models/rundiff-lagwe_1992-2011-sim-rgames.rds") %>%
    group_by(year) %>%
    mutate(id = 1:n()) %>%
    ungroup()
```
...
<!-- file: ../models/rundiff-lagwe_1992-2011-sim-cov.dat -->
...
```{r sim_wins, cache.extra = tools::md5sum("../models/rundiff-lagwe_1992-2011-sim-wins.rds")}
sim_wins <- readRDS("../models/rundiff-lagwe_1992-2011-sim-wins.rds") %>%
    mutate(yr = as.integer(as.character(yr)))
```

```{r pyth, cache.extra = tools::md5sum("../gamelogs/wins-pythagorean.csv")}
pyth <- read_csv("../gamelogs/wins-pythagorean.csv") %>%
    mutate(yr = as.integer(as.character(yr)))
```
...
```{r park_codes, cache.extra = tools::md5sum("../gamelogs/parkcode-cut.csv")}
park_codes <- read_csv("../gamelogs/parkcode-cut.csv",
...
```{r pids, cache.extra = tools::md5sum("../gamelogs/person-ids.csv")}
pids <-  read_csv("../gamelogs/person-ids.csv")
```
...
parameter is set to $\beta^{\text{home}}$), using posterior draws for
Wrigley Field's middle-of-the-pack scale parameter.  The estimate is
short by more than 2 percentage points.  If I strip the model of all
location predictors except for $\beta^{\text{home}}$, the estimate
remains low.

```{r eval = FALSE}
fit_bhome <- readRDS("../models/rundiff-home_1992-2011-fit.rds")
print(fit_bhome)
```
...
dos, but, as I discussed above, I think the home-field effect is being
...
[mdir]: https://github.com/kyleam/mlb-rundiff/tree/master/models
