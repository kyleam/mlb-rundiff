...
```{r lag, cache.extra = tools::md5sum("../output/lag/lag-combined-1990_2016.csv")}
...
```{r lag_ht, cache.extra = tools::md5sum("../output/lag/lag-combined-1990_2016-ht.csv")}
lag_ht <- read_csv("../output/lag/lag-combined-1990_2016-ht.csv") %>%
    mutate(date = ymd(date),
           year = year(date),
...
```{r s2_wide, cache.extra = tools::md5sum("../output/lag/song2017how-table-s2.csv")}
(s2_wide <- read_csv("../output/lag/song2017how-table-s2.csv") %>%
    mutate(date = ymd(date)))
```
...
around for at last three days, which is enough to drop their lag back
to zero.
