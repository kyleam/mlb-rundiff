...
```{r glog, dependson = "clean_team_name", cache.extra = tools::md5sum("../output/lag/log-with-lags-cleaned.csv")}
...
```{r info, cache.extra = tools::md5sum("../output/models/rundiff-lagwe_1992-2011.info.R")}
info <- read_rdump("../output/models/rundiff-lagwe_1992-2011.info.R")
```

```{r dat, cache.extra = tools::md5sum("../output/models/rundiff-lagwe_1992-2011.data.R")}
...
```{r fit, cache.extra = tools::md5sum("../output/models/rundiff-lagwe_1992-2011-fit.rds")}
...
```{r sim_rgames, cache.extra = tools::md5sum("../output/models/rundiff-lagwe_1992-2011-sim-rgames.rds")}
sim_rgames <- readRDS("../output/models/rundiff-lagwe_1992-2011-sim-rgames.rds") %>%
    group_by(year) %>%
    mutate(id = 1:n()) %>%
    ungroup()
```
...
<!-- file: ../output/models/rundiff-lagwe_1992-2011-sim-cov.dat -->
...
```{r sim_wins, cache.extra = tools::md5sum("../output/models/rundiff-lagwe_1992-2011-sim-wins.rds")}
sim_wins <- readRDS("../output/models/rundiff-lagwe_1992-2011-sim-wins.rds") %>%
    mutate(yr = as.integer(as.character(yr)))
```

```{r pyth, cache.extra = tools::md5sum("../output/wins-pythagorean.csv")}
pyth <- read_csv("../output/wins-pythagorean.csv") %>%
    mutate(yr = as.integer(as.character(yr)))
j```
...
```{r park_codes, cache.extra = tools::md5sum("../output/parkcode-cut.csv")}
park_codes <- read_csv("../output/parkcode-cut.csv",
...
```{r pids, cache.extra = tools::md5sum("../output/person-ids.csv")}
pids <-  read_csv("../output/person-ids.csv")
```
...
```{r eval = FALSE}
fit_bhome <- readRDS("../output/models/rundiff-home_1992-2011-fit.rds")
print(fit_bhome)
```
